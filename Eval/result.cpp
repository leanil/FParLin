...
int main() {
    ...
    std::cout << Map([&](const auto& a){return Fold([&](const auto& b){return [&](const auto& c){return (b)+(c);};},Zip([&](const auto& b){return [&](const auto& c){return (b)*(c);};},a,*bigVectors.at("vec")),0.000000);},make_vector({make_vector({1.000000,2.000000,3.000000}),make_vector({4.000000,5.000000,6.000000})})) << std::endl;
}